...
import { expectedProjectRowVersion } from './project.service';
...
export class ClarificationService {
  ...
  async answer(
    ...
    if (!projectId.success || !questionId.success) throw new NotFoundException('Project or clarification question was not found');
    const request = AnswerClarificationRequestSchema.safeParse(body);
    if (!request.success) throw new BadRequestException('Clarification answer must contain 1 to 2000 characters');
    ...
    try {
      ...
      if (cause instanceof ClarificationNotFoundError) throw new NotFoundException(cause.message);
      if (cause instanceof ClarificationVersionConflictError) throw new HttpException(cause.message, HttpStatus.PRECONDITION_FAILED);
